"""
Go 版 baseline 生成スクリプト — bare Qwen2.5-Coder-7B-Instruct で humaneval-go を解く。

kojiki_lm 依存なし。transformers.generate を直呼びする最小実装。

ロードマップ:
- M0 (Go 版): docs/roadmap_min_go.md の最初のマイルストーン
- 目的: Win Condition の baseline を確定する (修正後の言霊・Firewall の比較対象)
...
使い方:
    python3 scripts/eval/run_baseline_go.py \\
        ...
        --out-dir data/eval/generated/humaneval-go.baseline.seed0 \\
...
def parse_args():
    ...
    ap.add_argument("--input", required=True,
                    help="MultiPL-E parquet (humaneval-go / mbpp-go)")
